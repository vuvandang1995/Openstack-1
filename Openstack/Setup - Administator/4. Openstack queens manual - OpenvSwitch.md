...
firewall_driver = neutron.agent.linux.iptables_firewall.IptablesFirewallDriver (iptables_hybrid)
```
...
Cấu hình Linux bridge agent: `/etc/neutron/plugins/ml2/openvswitch_agent.ini`
...
`ip addr add 192.168.1.61/24 dev br-provider`
...
`ip addr add 192.168.1.62/24 dev br-provider`
