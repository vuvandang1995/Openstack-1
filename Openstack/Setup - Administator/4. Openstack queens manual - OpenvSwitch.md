# Cài đặt thủ công openstack phiên bản queens sử dụng openvswitch
...
```
. admin-openrc
openstack compute service list --service nova-compute2
```

VD:
```
[root@controller1 ~]# openstack compute service list --service nova-compute2
+----+--------------+----------+------+---------+-------+----------------------------+
| ID | Binary       | Host     | Zone | Status  | State | Updated At                 |
+----+--------------+----------+------+---------+-------+----------------------------+
|  6 | nova-compute | compute2 | nova | enabled | up    | 2018-07-09T07:48:45.000000 |
+----+--------------+----------+------+---------+-------+----------------------------+
```
...
`ip addr add 192.168.40.61/24 dev br-provider`
...
`ip addr add 192.168.40.62/24 dev br-provider`
